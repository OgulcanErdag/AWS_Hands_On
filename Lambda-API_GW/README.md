...
logger.info("Lambda function is loading")
...
def lambda_handler(event, context):
    ...
    for i, d in enumerate(sayi):
        while myNumber >= d:
            ...
            romanvalue += roman[i]
            logger.info(
                f"Matched {d}, remaining number: {myNumber}, current roman: {romanvalue}"
            )

    logger.info(f"Final Roman result for {number}: {romanvalue}")
