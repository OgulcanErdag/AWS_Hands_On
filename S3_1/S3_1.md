...
Create S3 buckets, understand object/public access models, host a static website, and practice versioning & basic logging.

## Learning Outcomes

At the end of the this hands-on training, students will be able to;

- Create an S3 bucket (naming, region).

- Upload, move, rename (copy+delete), and delete objects.

- Make objects publicly readable using bucket policy (recommended) or ACL (demo).

- Host a static website from S3 and understand HTTPS note (CloudFront).

- Enable versioning and work with delete markers and previous versions.

- Enable server access logging basics.
...
- Create two buckets for pratik (we’ll use one for ACL demo, one for policy demo):

```text
Bucket name                 : myfirstbucket-<your-name>-01
...
- Create a new bucket for static website with name of `pet.static.web.hosting` and with following properties

```text
Bucket name                 : pet.static.web.hosting
Region                      : N.Virginia
...
- Click the S3 bucket `pet.static.web.hosting` and upload following files.
...
- Show static website hosting settings from properties of `pet.static.web.hosting` bucket.
...
- Create folder called `kitten` under the bucket named `pet.static.web.hosting`.
...
- Create a new bucket named `pet.versioning` with following properties.

```text
Bucket name                 : pet.versioning
Region                      : N.Virginia
Object Ownership            : ACLs disabled (recommended)****
...
- Click the S3 bucket `pet.versioning` and upload following files.
...
- Show static website hosting settings from properties of `pet.versioning` bucket.
...
- Show versioning of the files right under the bucket `pet.versioning`
