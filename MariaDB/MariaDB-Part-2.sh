...
# No root password for root so 'Enter' for first question, then say "n" to "Switch to unix_socket authentication [Y/n]"
...
# Create new database named 'companydb'.
CREATE DATABASE companydb;

# Show newly created database.
SHOW DATABASES;

# Create a user named 'companyuser'.
CREATE USER companyuser IDENTIFIED BY 'company1234';

# Grant permissions to the user companyuser for database companydb.
GRANT ALL ON companydb.* TO companyuser IDENTIFIED BY 'company1234' WITH GRANT OPTION;

# Update privileges.
FLUSH PRIVILEGES;

# Check permission.
SHOW GRANTS FOR 'companyuser'@'%';
