...
```text
Security Group Name  : EC2 SecGrp-SSH
...
```bash
sudo hostnamectl set-hostname ogulcanFirst
```
...
sudo mount -t efs -o tls fs-03c2e624f2b975b8c:/ efs
...
```bash
sudo hostnamectl set-hostname ogulcanSecond
```
...
sudo mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport fs-03c2e624f2b975b8c.efs.us-east-1.amazonaws.com:/ efs
```
...
#### If you dont want to use "Automatic" feauture on AWS management console, you can mount as written below
...
fs-xxxxxx:/ /home/ec2-user/efs efs tls,_netdev,,noresvport,tls,iam 0 0

fs-03c2e624f2b975b8c:/ /home/ec2-user/efs efs tls,_netdev,tls 0 0
...
```bash
fs-03c2e624f2b975b8c.efs.us-east-1.amazonaws.com:/ /home/ec2-user/efs nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport,_netdev 0 0
```
...
```bash
sudo hostnamectl set-hostname ogulcanThird
```
