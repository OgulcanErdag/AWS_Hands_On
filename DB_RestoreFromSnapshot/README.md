...
- For `point in time recovery` PART (PART-4), please note down the current time. (Exp: November 27, 2025, 10:16:02, UTC +3) and let the students know:

  -enter the time: 10:16:02
...
👉 **If your account already has one running RDS instance,**  
you will see the following error:

![Free Tier Error](./free_tier.png)

**“You reached the maximum number of instances available with free plan accounts.”**

### ✔️ Solution

Before restoring, **delete every RDS instance except the original snapshot source**, including:

- restored-from-man-snapshot
- restored-from-point-in-time-RDS
- any leftover test instances

### ✔️ Why must I delete them?

Because Free Tier only allows **ONE** RDS instance.  
Restoring always creates a **new** one → which exceeds the limit.

Once you delete the extra instances, you can continue:

➡️ **Actions → Restore to point in time**
